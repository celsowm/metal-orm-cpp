...
struct [[=metal::mapping::table{"pg_types"}]] PostgresTypes {
    [[=metal::mapping::primary_key, =metal::mapping::generated]]
    std::int64_t id{};
    bool active{};
    double ratio{};
...
int main() {
    ...
    assert(metal::create_table_sql<SchemaUser>(postgres) ==
        "CREATE TABLE IF NOT EXISTS \"actual_users\" ("
        "\"id\" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
        "\"name\" TEXT NOT NULL, \"age\" BIGINT);");
    assert(metal::create_table_sql<PostgresTypes>(postgres) ==
        "CREATE TABLE IF NOT EXISTS \"pg_types\" ("
        "\"id\" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
        "\"active\" BOOLEAN NOT NULL, \"ratio\" DOUBLE PRECISION NOT NULL, "
        "\"payload\" BYTEA NOT NULL, \"note\" TEXT);");
    const auto postgres_post_ddl = metal::create_table_sql<SchemaPost>(postgres);
    assert(postgres_post_ddl.find(
        "\"user_id\" BIGINT REFERENCES \"actual_users\" (\"id\") ON DELETE CASCADE") !=
        std::string::npos);
    assert(postgres_post_ddl.find("AUTOINCREMENT") == std::string::npos);

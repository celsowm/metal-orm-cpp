...
struct relation_info {
    ...
    static consteval std::meta::info target_key() {
        ...
        } else if constexpr (kind == mapping::relation_kind::belongs_to) {
            return reflect::key_or_primary<target_type>(traits::target_key());
        } else if constexpr (kind == mapping::relation_kind::has_many ||
                             kind == mapping::relation_kind::has_one ||
...
consteval bool graph_value_compatible() {
    ...
    if constexpr (is_optional_v<M>) {
        ...
        if constexpr (is_optional_v<V>) {
            return graph_value_compatible<Inner, typename V::value_type>();
        }
        return graph_value_compatible<Inner, Input>();
...
bool missing_key(const T& entity) {
...
std::shared_ptr<T> find_by_key(Session& session, const Value& key) {
    ...
    constexpr auto pk = reflect::primary_key_member<T>();
    if constexpr (Key == pk) {
        return session.find<T>(typed);
    } else {
        return session.query<T>().where(field<Key> == typed).first();
    }
}

template <reflect::Entity T, std::meta::info Key>
std::shared_ptr<T> reference_by_key(Session& session, const Value& key, bool track_stub) {
    if (auto found = find_by_key<T, Key>(session, key)) return found;
    auto target = std::make_shared<T>();
    using K = reflect::member_type_t<Key>;
    target->[:Key:] = from_value<K>(key);
    if (track_stub) session.persist(target);
    return target;
}
...
void apply_single_key(Session& session, Root& root, const Value& key, const GraphOptions& options);

template <std::meta::info Relation, reflect::Entity Root>
void clear_single(Session& session, Root& root, const GraphOptions& options);
...
public:
    ...
    struct item {
        ...
        std::conditional_t<std::is_void_v<pivot_type>, std::monostate, pivot_patch<pivot_type>> pivot{};
    ...
    graph_collection_input& add_id(Key&& key) {
        ...
        static_assert(graph_detail::graph_value_compatible<K, Key>(),
...
public:
    ...
    graph_payload& set(V&& value) {
        ...
        static_assert(graph_detail::graph_value_compatible<M, V>(),
        ...
        scalar_actions_.push_back([converted = std::move(converted)](T& entity) mutable {
    ...
    graph_payload& relation(graph_payload<Target> payload) {
        validate_relation_member<Relation>();
        using Info = graph_detail::relation_info<Relation>;
        if constexpr (Info::kind == mapping::relation_kind::morph_to) {
            using Wrapper = reflect::member_type_t<Relation>;
            static_assert(reflect::morph_to_reference_traits<std::remove_cvref_t<Wrapper>>::template contains<Target>,
                          "MetalORM: graph morph_to target is not declared by the relation wrapper");
            pre_relations_.push_back(
                [payload = std::move(payload)](Session& session, T& root, const GraphOptions& options) {
            ...
            static_assert(std::same_as<typename Info::target_type, Target>,
                          "MetalORM: graph relation payload type does not match the reflected relation target");
            ...
            if constexpr (Info::kind == mapping::relation_kind::belongs_to) pre_relations_.push_back(std::move(action));
            else post_relations_.push_back(std::move(action));
        }
        return *this;
    }
    ...
    graph_payload& relation_id(Key&& key) {
        validate_relation_member<Relation>();
        using Info = graph_detail::relation_info<Relation>;
        static_assert(Info::kind != mapping::relation_kind::morph_to,
                      "MetalORM: morph_to relation_id requires a concrete target payload; use relation<>()");
        static_assert(Info::kind == mapping::relation_kind::belongs_to ||
                      ...
                      "MetalORM: relation_id is only for single-target relations");
        constexpr auto key_member = Info::target_key();
        using K = reflect::member_type_t<key_member>;
        static_assert(graph_detail::graph_value_compatible<K, Key>(),
                      "MetalORM: graph relation id is incompatible with the relation target key");
    ...
    graph_payload& clear_relation() {
        ...
        static_assert(Info::kind == mapping::relation_kind::belongs_to ||
                      ...
                      "MetalORM: clear_relation is only for single-target relations");
        ...
        if constexpr (Info::kind == mapping::relation_kind::belongs_to ||
                      Info::kind == mapping::relation_kind::morph_to) {
            pre_relations_.push_back(std::move(action));
        } else {
            post_relations_.push_back(std::move(action));
        }
        return *this;
    }
    ...
    requires std::invocable<Configure, graph_collection_input<Relation>&>
    {
        validate_relation_member<Relation>();
        using Info = graph_detail::relation_info<Relation>;
        static_assert(Info::kind == mapping::relation_kind::has_many ||
                      Info::kind == mapping::relation_kind::many_to_many ||
                      Info::kind == mapping::relation_kind::morph_many,
                      "MetalORM: collection graph callback requires has_many, many_to_many or morph_many");
        graph_collection_input<Relation> input;
        std::invoke(std::forward<Configure>(configure), input);
        post_relations_.push_back(
            [input = std::move(input)](Session& session, T& root, const GraphOptions& options) {
    ...
    bool has_post_relations() const noexcept { return !post_relations_.empty(); }

    void apply_scalars(T& entity) const {
        for (const auto& action : scalar_actions_) action(entity);
    }

    void apply_pre(Session& session, T& entity, const GraphOptions& options) const {
        for (const auto& action : pre_relations_) action(session, entity, options);
    }

    void apply_post(Session& session, T& entity, const GraphOptions& options) const {
        for (const auto& action : post_relations_) action(session, entity, options);
    }

private:
    template <std::meta::info Relation>
    static consteval void validate_relation_member() {
    ...
    std::vector<scalar_action> scalar_actions_;
    std::vector<relation_action> pre_relations_;
    std::vector<relation_action> post_relations_;
};
...
std::shared_ptr<T> materialize(Session& session, const graph_payload<T>& payload, const GraphOptions& options) {
    ...
    const bool needs_generated_root_key = tracked && tracked->status == EntityStatus::New &&
        reflect::primary_key_is_generated<T>() && payload.has_post_relations();
    if (needs_generated_root_key) session.flush();
...
void apply_single_payload(Session& session, Root& root, const graph_payload<Target>& payload, const GraphOptions& options) {
    ...
    if constexpr (Info::kind == mapping::relation_kind::belongs_to) {
        ...
        if (missing_key<Target, target_key>(*target)) session.flush();
        if (missing_key<Target, target_key>(*target)) {
...
void apply_single_key(Session& session, Root& root, const Value& key, const GraphOptions&) {
    ...
    auto target = reference_by_key<Target, target_key>(session, key, Info::kind != mapping::relation_kind::belongs_to);
...
void clear_nullable(Owner& owner) {
    if constexpr (is_optional_v<Member>) owner = std::nullopt;
    else throw std::runtime_error("MetalORM: graph relation clear requires a nullable foreign key or morph field");
}
...
void clear_single(Session& session, Root& root, const GraphOptions&) {
    ...
    if constexpr (Info::kind == mapping::relation_kind::belongs_to) {
        constexpr auto foreign_key = Traits::foreign_key();
        using FK = reflect::member_type_t<foreign_key>;
        auto previous = root.[:Relation:].get();
        if constexpr (mapping::cascades_remove(Traits::cascade)) {
            if (previous) session.remove(previous);
        }
        ...
        if (previous) {
            if constexpr (mapping::cascades_remove(Traits::cascade)) session.remove(previous);
                ...
                if constexpr (is_optional_v<FK>) {
                    ...
                    throw std::runtime_error("MetalORM: clearing has_one requires nullable target FK or cascade remove");
                }
            }
        }
        root.[:Relation:].reset();
        ...
        if (previous) {
            if constexpr (mapping::cascades_remove(Traits::cascade)) session.remove(previous);
                ...
                if constexpr (is_optional_v<Id> && is_optional_v<Type>) {
                    ...
                    throw std::runtime_error("MetalORM: clearing morph_one requires nullable morph fields or cascade remove");
                }
            }
        }
        root.[:Relation:].reset();
        ...
        if constexpr (is_optional_v<Id> && is_optional_v<Type>) {
            ...
            throw std::runtime_error("MetalORM: clearing morph_to requires nullable id/type fields");
        }
        root.[:Relation:].reset();
    }
}
...
void apply_morph_to_payload(Session& session, Root& root, const graph_payload<Target>& payload, const GraphOptions& options) {
    ...
    if (!matched) throw std::logic_error("MetalORM: graph morph_to target is not declared by the relation mapping");
}
...
void apply_collection(Session& session, Root& root, const graph_collection_input<Relation>& input, const GraphOptions& options) {
    ...
    auto remember = [&](const std::shared_ptr<Target>& target) {
        if (!target || missing_key<Target, key_member>(*target)) return;
    ...
    for (const auto& item : input.items()) {
        ...
        if (item.payload) target = materialize(session, *item.payload, options);
            ...
            if constexpr (Info::kind == mapping::relation_kind::many_to_many) {
                if (item.has_pivot) collection.attach(from_value<reflect::member_type_t<key_member>>(*item.key), item.pivot);
                else collection.attach(from_value<reflect::member_type_t<key_member>>(*item.key));
                if (auto found = find_by_key<Target, key_member>(session, *item.key)) remember(found);
                else seen.push_back(value_key(*item.key));
                continue;
            } else {
                target = reference_by_key<Target, key_member>(session, *item.key, true);
            }
        }
        if (!target) continue;

        if constexpr (Info::kind == mapping::relation_kind::many_to_many) {
            if (item.has_pivot) collection.attach(target, item.pivot);
            else collection.attach(target);
        } else {
            bool present = false;
            for (const auto& current : collection.get_items()) {
                if (current == target) { present = true; break; }
                if (current && !missing_key<Target, key_member>(*current) &&
                    !missing_key<Target, key_member>(*target) &&
                    value_key(to_value(current->[:key_member:])) == value_key(to_value(target->[:key_member:]))) {
                    present = true;
                    break;
                }
            }
    ...
    const auto desired = [&](const std::shared_ptr<Target>& target) {
        if (!target || missing_key<Target, key_member>(*target)) return true;
